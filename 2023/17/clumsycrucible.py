...
# This is a path finding problem
# Dijkstra algo
def findLeastHeatPathUltraCrucible(data):
    h = len(data)
    w = len(data[0])
    ...
    while True:
        ...
        for astate in states:
            ...
            # Continue straight
            # The last straight line must be <= 10 but >= 4
            roomCanStopAtEnd = True
            if direction == kDown and x==w-1:
                distanceToEnd = h-1-y
                roomCanStopAtEnd = ((distance+distanceToEnd) <= 10) or distanceToEnd > 4
            elif direction == kRight and y==h-1:
                distanceToEnd = w-1-x
                roomCanStopAtEnd = ((distance+distanceToEnd) <= 10) or distanceToEnd > 4

            if distance < 10 and roomCanStopAtEnd:
                res = addState(x+kMoves[direction]['x'], y+kMoves[direction]['y'], currentcost, direction, distance+1)
                if res>0:
                    return res
...
print(f"Problem 2: Heat={findLeastHeatPathUltraCrucible(data)}")
